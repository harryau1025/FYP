...
goal = 419
SCS_MINIMUM_POSITION_VALUE  = 330       # SCServo will rotate between this value
SCS_MAXIMUM_POSITION_VALUE  = 330     # and this value (note that the SCServo would not move when the position value is out of movable range. Check e-manual about the range of the SCServo you use.)
...
while 1:
    ...
    while 1:
        ...
        #   break
        if (abs(goal - scs_present_position)) < SCS_MOVING_STATUS_THRESHOLD or (counter==0 and load >= 500 and temp_count == 1000) or (counter == 1 and load >= 1500 and temp_count ==1000):
            break
